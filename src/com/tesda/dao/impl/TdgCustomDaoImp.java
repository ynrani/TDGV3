			...
			if (testDataGenenarteDTO.getSchemamastertables() != null
				...
				if (strArrays.length >= 1) {
					if (strArrays[0].contains(",")) {
						...
						System.out.println(strArrays[0].toUpperCase().trim());
					}
				}
			}
...
*/									lstIdentifiedcolumns.add(String.valueOf(mapResult.get("COLUMN_NAME")));
								...
								if(listAutoincrementColumns != null && !listAutoincrementColumns.isEmpty()){
									for(Map<String,Object> mapObject : listAutoincrementColumns){
										if(mapObject.get("COLUMN_NAME").toString().equalsIgnoreCase(mapFinalCols.getKey()) && mapObject.get("TABLE_NAME").toString().equalsIgnoreCase(strTableName))
											...
											if(testDataGenenarteDTO.isRequiredAllColumns()){
												mapPositions.put(mapFinalCols.getValue(), mapFinalCols.getKey());
											strInsertQuery.append(mapFinalCols.getKey()).append(" , ");
											}else{
						...
						System.out.println("---strInsertQuery----"+strInsertQuery.toString());
						System.out.println("-----"+mapTableWithPk.toString());
			...
			if (!isBreaked) {
				...
				for (int i = iHashValue; i >= 1; i--) {
					System.out.println(" listResultVal.get "+listResultVal.get(iIteratorCount));
					...
					for (String tbName : listFinalSequence) {
						/*to remove hard-coded dbo and adding code for sqlserver bcoz schema.table name*/
					/*	if(tbName.contains(".")){
						String[] tabsWithSchema = tbName.split("[.]");
						tbName = tabsWithSchema[1];
							till here
						}*/
						...
						if (!mapPks.containsKey(tbName) && !listMasterTabs.contains(tbName)
							...
							/**
							 * Going to generate values based on constraint types which TDG Engine rectifies
							 */
							...
							if (!mapTableWithSequence.containsKey(tbName) && !vct.isEmpty()) {
								...
								strPrimaryKey = TdgCentralConstant.getSpecificDBQuery(tdgTemplateDao.getDbType(),
										TdgCentralConstant.PRIMARY_KEY_FINAL, vct);
							}
							
							if (mapConditions.containsKey(mapConditionWithPKs.get(tbName))) {
								//added below code for DB Seq support
								if(String.valueOf(mapConditions.get(mapConditionWithPKs.get(tbName))).contains(TdgCentralConstant.TDG_DB_SEQ)){
									String strInputValue = String.valueOf(mapConditions.get(mapConditionWithPKs.get(tbName))).substring(10);
									if(!StringUtils.isNotEmpty(strInputValue)){
									
									Vector<String> vt = new Vector<String>();
									vt.add(mapConditionWithPKs.get(tbName));
									vt.add(tbName);
									String strDbSeq = TdgCentralConstant.getSpecificDBQuery(tdgTemplateDao.getDbType(),
											TdgCentralConstant.MAX_COLUMN_VALUE, vt);
									/*vctVal.addAll(tdgTemplateDao.generatePkValues(jdbcTemplate,
									...
									if(strVal.matches(regexForNumbers))
										strDbSeq = TdgCentralConstant.getSpecificDBQuery(tdgTemplateDao.getDbType(),
												TdgCentralConstant.PRIMARY_KEY_FINAL, vt);
									vctValues.addAll(TdgAutoIncrementUtil.generateAutoIncrementValues(tdgTemplateDao.getMaxValue(jdbcTemplate,strDbSeq), (long)lRecordsCount+1));
									vctValues.remove(0);
									}else{
										vctValues.addAll(TdgAutoIncrementUtil.generateAutoIncrementValues(strInputValue, (long)lRecordsCount));
									}
								}else{
								//end of support
									//below code whether code generated already regexsequence values or not 
									try{
									@SuppressWarnings("rawtypes")
									List conditionCheckValues = (ArrayList) mapConditions.get(mapConditionWithPKs.get(tbName));
									vctValues.addAll(conditionCheckValues);
									}catch(Exception e){
							...
							} else if (StringUtils.isNotEmpty(strPrimaryKey) || mapTableWithSequence.containsKey(tbName)) {
								Map<String, Map<String, String>> mapPkWithCol = mapTableWithSequence
										.get(tbName);
								//below filter added to check already generated keys from Unique key constraints
								Map<String, Map<String, String>> mapFinalFilter = new HashMap<String, Map<String, String>>();
								if(mapPkWithCol != null && !mapPkWithCol.isEmpty())
								for(Map.Entry<String,  Map<String, String>> entrySetvalues : mapPkWithCol.entrySet()){
									if(!lstGeneratedValues.contains(entrySetvalues.getKey()))
										mapFinalFilter.put(entrySetvalues.getKey(), entrySetvalues.getValue());
								}
								if (mapFinalFilter != null && !mapFinalFilter.isEmpty()) {
									Map<String, String> mapSequeces = mapFinalFilter.get(mapPositions
											.get(1));
									...
									//mapRes.put(tbName, mapSequences);
									vctValues.addAll(tdgTemplateDao.generatePkValues(jdbcTemplate,
											null, mapFinalSequence, lRecordsCount, tbName));
								} else if(StringUtils.isNotEmpty(strPrimaryKey) ) {
									//commented code for db seq value generation dynamically
									/*vctValues.addAll(tdgTemplateDao.generatePkValues(jdbcTemplate, strPrimaryKey,
											null, lRecordsCount, tbName));*/
									String strDbSeq = TdgCentralConstant.getSpecificDBQuery(tdgTemplateDao.getDbType(),
											TdgCentralConstant.MAX_COLUMN_VALUE, vct);
									System.out.println("string deb"+strDbSeq);
									/*vctVal.addAll(tdgTemplateDao.generatePkValues(jdbcTemplate,
											strDbSeq, null, 1, tbName));*/
									String strVal = tdgTemplateDao.getMaxValue(jdbcTemplate,
											strDbSeq);
									System.out.println("string value is "+strVal);
									if(strVal.matches(regexForNumbers))
										strDbSeq = TdgCentralConstant.getSpecificDBQuery(tdgTemplateDao.getDbType(),
												TdgCentralConstant.PRIMARY_KEY_FINAL, vct);
									vctValues.addAll(TdgAutoIncrementUtil.generateAutoIncrementValues(tdgTemplateDao.getMaxValue(jdbcTemplate,strDbSeq), (long)lRecordsCount+1));
                           ...
                           // List<Object[]> listParameters = null;
                            for(long i2 = 0;i2 < iLoop;i2++){
                                ...
                                for(int iRecord = 0;iRecord < iNoOfTimes;iRecord++){
        							...
        							//for (int iRecords = 0; iRecords < lRecordsCount; iRecords++) {
        								...
        								if (listRelations != null) {
        									...
        									System.out.println("values passing are"+obj.toString());
        								}
        							//}
        							bUniqCheck = false;
        							if (logger.isDebugEnabled()) {
    							 ...
    							 */
    								...
    								System.out.println("-0-----strQuery----"+strQuery);
    								System.out.println("--0000000-----listParameters-----"+listParameters.toString());
    								try{
